...
public class BatchReviewConvertProcess<T extends DatabaseProducer.Row<?>> implements Runnable {
	...
	private void batchInsert() throws InterruptedException {
		...
		ReviewProcedures.addReviewAndReviewerBatch(reviewElements);
		...
		try {
			while (scrape != null) {
				...
				try {
					...
					Boot.getLogger().log(e, "Trouble with scrape2profileElement. Skipped scrape id " + scrape.id + ", URL " + scrape.url);
				}
			...
			Boot.getLogger().log(e, "InterruptedException. Skipped scrape id " + scrape.id 
								  + ", URL " + scrape.url);
		}
