...
public class BatchUserConvertProcess<T extends DatabaseProducer.Row<?>> implements Runnable {
	...
	public BatchUserConvertProcess(DatabaseProducer<Scrape, Integer> producer, 
	...
	public void run() {
		...
		try {
			while (scrape != null) {
				try {
					ProfileElement pe = scrape2profileElement(scrape);
	...
	private ProfileElement scrape2profileElement(Scrape scrape) throws Exception {
